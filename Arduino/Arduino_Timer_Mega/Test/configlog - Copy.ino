void configlog (byte datalogset, float CutOffVolts, float CutOffApms, byte SogMorning, byte SogNight, byte NoOfTimers, int onTime1, int offTime1, int onTime2, int offTime2, int onTime3, int offTime3, int onTime4, int offTime4, int onTime5, int offTime5, int offset0, int offset1, int offset2, int offset3, int offset4, int offset5, int offset6, int offset7, float Callf0, float Callf1, float Callf2, float Callf3, float Callf4, float Callf5, float Callf6, float Callf7)
{
  if (datalogset == 1)
  {
    if (Serial1.find("+IPD"))
    {
      Serial.println("HTTP request recived");
      ...
      cmd += data.length();
      cmd += "\r\n";
      SendCmd(cmd,100,true);
      SendCmd(data,100,true);
      cmd = "AT+CIPCLOSE=";
      cmd +=cid;
      cmd += "\r\n";
      SendCmd(cmd,100,true);
