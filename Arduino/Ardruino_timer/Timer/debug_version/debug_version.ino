...
void loop() 
{
  ...
  Alarm(17,03);                    // 6:00am every day (xx,xx,0,preset) values denoted by x can be modified to change trigger time
  Alarm(18,27);                   // 6:00pm every day (xx,xx,0,preset) values denoted by x can be modified to change trigger time
...
void preset()
{
  unsigned long dtimeM = 5; // Change this value to desired On time 
  digitalWrite(13, HIGH);  // turn the LED/Relay on (HIGH is the voltage level)  
  boolean relay_on = true;
  unsigned long dtime = 300000; // Calculating delay time based of pot position in milliseconds 
  unsigned long currentMillis = millis();
  unsigned long previousMillis = currentMillis;
  while (relay_on == true)
    {
