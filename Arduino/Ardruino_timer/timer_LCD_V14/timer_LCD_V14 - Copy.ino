...
void loop()
{
  ...
  while (1)
  {
    PageNo = update_btn(button(), PageNo, 10, 50);
    ...
    PageNo = update_btn(button(), PageNo, 10, 10);
    if (RelayOn == 0)
    {
      pelaptime = millis();
      elaptime = 0;
    ...
    if (NoOfTimers == 0)
    {
      ...
      if (RunCount >= 100)
      {
        RelayOn = deactivate_relay(amps, CutOffApms, SogMorning, SogNight, RelayOn, elaptime, 0);
      }
    }
    for (int i = 0; i < NoOfTimers; i++)
    {
    ...
    datalog(datalogset, RelayOn, amps, volts, ampsac, OnDay, OnMonth, OnHour, OnMinute, KWH, GridTime, CutOffVolts, CutOffApms, SogMorning, SogNight, sysvoltsdc, SysampsDC, SysampsAC, NoOfTimers, ACOffset, DcOffset, ACcalf, DCcalf, ACerror, onTime[0], offTime[0], onTime[1], offTime[1], onTime[2], offTime[2], onTime[3], offTime[3], onTime[4], offTime[4]);
    if (GridTime > 4294966296 || KWH > 4294966296 )
    {
      GridTime = 0;
      KWH = 0;
      OnDay = day();
